...
### 新增
- **字幕工坊** - 新增统一的字幕工作区，支持 SRT/VTT/ASS，并集成核心编辑能力（时间轴、查找替换、自动修复）、URL 字幕下载与 AI 功能（翻译、语法修正、Whisper 生成）
- **后端 AI 响应命令** - 新增 `generate_ai_response` Tauri 命令，读取已保存的 AI 配置并调用相应提供商，使前端 AI 功能得以实现
- **字幕 Phase 1 核心工具** - 新增波形/频谱时间轴拖拽校时、实时质检指标（CPS/WPM/CPL + 时长/间隔/重叠）、高级拆分合并工具，以及可保留原文的双栏翻译模式（原文/译文）
