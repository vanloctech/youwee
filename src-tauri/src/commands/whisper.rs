...
pub async fn transcribe_video_with_whisper(
    ...
    add_log_internal(
        "info",
        &format!("Starting Whisper transcription for: {}", video_path),
        None,
        None,
    )
    .ok();
    ...
    let audio_path = if ["mp4", "mkv", "webm", "avi", "mov", "flv"].contains(&extension.as_str()) {
        ...
        extract_audio_for_whisper(&video_path, &audio_output_str, ffmpeg_path_str.as_deref())
            .await
...
pub async fn transcribe_url_with_whisper(
    ...
    add_log_internal(
        "info",
        &format!("Starting Whisper transcription for URL: {}", url),
        None,
        Some(&url),
    )
    .ok();
    ...
    if metadata.len() > max_size {
        add_log_internal(
            "info",
            "Audio file too large, compressing...",
            None,
            Some(&url),
        )
        .ok();
        ...
        extract_audio_for_whisper(&audio_file, &compressed_str, ffmpeg_path_str.as_deref())
            .await
            .map_err(|e| e.to_string())?;
